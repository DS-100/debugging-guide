...
```{image} images/sql_error.png
:width: 700
:alt: Screenshot of a SQL Error message with the most important parts annotated. After 'Original error message from DB driver' is the highlighted error type followed by a description of why that error occured. Next in the error message is the line of code that caused the error.
```
...
```{image} images/imdb_schema.png
:width: 700
:alt: Schema for the IMDB database
```
